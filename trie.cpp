...
void deleteWord(char str[], int index, struct Trie_Node*& cur, int len){
    if(len==index){
        ...
        int c = index;
        deleteWord(str,++c,cur->character[str[index]],len);

        if(cur->character[str[index]] == NULL ){
            cur->character.erase(str[index]);

            if(cur->character.empty() && !cur->endofword){
                delete cur;
                cur=nullptr;
            }
        }
